function handle=scaledcolorbar(arg1, arg2, arg3)
...
if loc(1)=='v' % create or refresh vertical colorbar
    ...
    if isempty(ax)
        ...
        chwidth = 0.05;
        stripe = 0.075 - chwidth ; edge = 0.02; 
        if all([az,el]==[0 90])
            space = 0.05;
        else
            space = .1;
...
elseif loc(1)=='h', % create or refresh horizontal colorbar
    ...
    if isempty(ax),
        ...
        % chwidth = 0.05;
        chwidth = 0.02;
        stripe = 0.05 - chwidth; space = 0.1;
        
        set(h,'Position',...
            [pos(1) pos(2)+(stripe + chwidth + space)*pos(4) pos(3) (1-stripe - chwidth - space)*pos(4)])
        legend('RecordSize',h); %set this as the new legend fullsize
